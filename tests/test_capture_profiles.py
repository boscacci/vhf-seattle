...
def test_elliott_bay_profile_monitors_two_nearby_channels() -> None:
...
def test_elliott_bay_airband_config_can_stream_both_monitored_channels() -> None:
    config = render_rtlsdr_airband_config(
        CAPTURE_PROFILES["elliott_bay"],
        output_root="/opt/talkingboats/spool/airband",
        icecast_outputs=(
    ...
    assert config.count('type = "icecast";') == 2
    assert config.count("continuous = true;") == 2
    assert 'label = "vhf-13";' in config
